...
static int invoke_commands(struct cmd *cmdhead) {
  ...
  // 下の2行が本質的で、それ以外はstdin stdoutを一時的に避難して、
  // 最後に元に戻してる？
...
static void redirect_stdout(char *path) {
  // ファイル名pathへの書き込みができるファイルディスクリプタを作成し
  // 標準出力の番号として書き換える
  int fd;
  ...
  while (*p) {
    ...
    if (*p && IDENT_CHAR_P(*p)) {
      if (cmd->capa <= cmd->argc) {
        cmd->capa *= 2;
        cmd->argv = xrealloc(cmd->argv, cmd->capa * sizeof(char*));
      }
      cmd->argv[cmd->argc] = p;
      cmd->argc++;
    }
    while (*p && IDENT_CHAR_P(*p)) { p++; }
  }
