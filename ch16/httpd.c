...
typedef void (*sighandler_t)(int); // sighandler_t : *(int -> void)
...
static void method_nod_allowed(struct HTTPRequest *req, FILE *out);
...
static void
method_nod_allowed(struct HTTPRequest *req, FILE *out) {
...
output_common_header_fields(struct HTTPRequest *req, FILE *out, char *status) {
  ...
  strftime(buf, TIME_BUF_SIZE, "%a, %d %d %Y %H %M:%S GMT", tm);
